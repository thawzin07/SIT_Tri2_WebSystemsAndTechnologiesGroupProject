<section class="container page-shell">
  ...
  <?php else: ?>
    <div class="table-responsive">
      <table id="bookings-table" class="table table-sm table-striped align-middle">
        <thead><tr><th>ID</th><th>Member</th><th>Email</th><th>Class</th><th>Date</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody>
          <?php foreach ($bookings as $b): ?>
            <tr>
              ...
              <td><?= e($b['email']) ?></td>
              <td><?= e($b['title']) ?></td>
              <td><?= e($b['class_date']) ?> <?= e(substr($b['start_time'],0,5)) ?></td>
              <td><?= e($b['booking_status']) ?></td>
              <td class="d-flex gap-1">
                <form action="/admin/bookings/update" method="post" class="d-flex gap-1">
                  <?= csrf_input() ?>
                  <input type="hidden" name="id" value="<?= (int)$b['id'] ?>">
                  <select name="booking_status" class="form-select form-select-sm">
                    <option value="booked" <?= $b['booking_status']==='booked'?'selected':'' ?>>booked</option>
